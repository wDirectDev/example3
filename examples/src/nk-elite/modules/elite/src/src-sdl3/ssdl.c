...
SDL_IOStream *datafile_open ( const char *fn )
{
	const Uint8 *data_p;
	int data_size;
	datafile_select(fn, &data_p, &data_size);
	SDL_IOStream *v = SDL_IOFromConstMem(data_p, data_size);
	if (!v) {
...
static void load_sprite ( int i, const char *fn, SDL_Surface **pass_surface_back )
{
	...
	if (i & IS_IMG_EXTERNAL) {
		...
		surface = SDL_LoadBMP_IO(datafile_open(fn), 1);
		if (!surface) {
	...
	rc = SDL_GetTextureSize(sprites[i].tex, &sprites[i].rect.w, &sprites[i].rect.h);
	if (!rc) {
...
{
	SDL_Palette *palette;
	SDL_PixelFormatDetails *pixfmt;
	SDL_bool rc;

	int _count = SDL_GetNumRenderDrivers();
	for ( int i = 0; i < _count; i++ ) {
		const char * _name = SDL_GetRenderDriver( i );
	...
	sdl_ren = SDL_CreateRenderer(sdl_win, "opengles2");
	if (!sdl_ren) {
		ERROR_WINDOW("Cannot create renderer: %s", SDL_GetError());
		return 1;
	}

	SDL_SetRenderLogicalPresentation(sdl_ren, SCREEN_W, SCREEN_H, SDL_LOGICAL_PRESENTATION_DISABLED, SDL_SCALEMODE_NEAREST );
	pixfmt = SDL_GetPixelFormatDetails(PIXEL_FORMAT);
	if (!pixfmt) {
	...
	rc = SDL_SetRenderTarget(sdl_ren, sdl_tex);
	if (!rc) {
...
//	}
	...
	for (int a = 0; a < 0x100; a++) {
		the_palette_r[a] = palette->colors[a].r;
		the_palette_g[a] = palette->colors[a].g;
		the_palette_b[a] = palette->colors[a].b;
...
//	scanner_texture = sprites[0].tex;
	...
	surface = SDL_LoadBMP_IO(datafile_open("icon.bmp"), 1);
	if (surface) {
	...
	//gfx_draw_scanner();
	SDL_RenderTexture(sdl_ren, sprites[IMG_THE_SCANNER].tex, NULL, &sprites[IMG_THE_SCANNER].rect);	// render scanner without setting clipping (would be with gfx_draw_scanner ...)
...
static ETNK_INLINE void set_clip ( int x1, int y1, int x2, int y2 )
{
	...
	SDL_SetRenderClipRect(sdl_ren, &rect);
}
...
void gfx_draw_scanner (void)
{
	...
	SDL_RenderTexture(sdl_ren, sprites[IMG_THE_SCANNER].tex, NULL, &sprites[IMG_THE_SCANNER].rect);
#if 0
	set_clip(/*gfx_screen,*/ GFX_X_OFFSET, 385 + GFX_Y_OFFSET,
...
void gfx_draw_sprite ( int sprite_no, int x, int y )
{
	...
	SDL_RenderTexture(sdl_ren, sprites[sprite_no].tex, NULL, &sprites[sprite_no].rect);
}
	...
	while (SDL_PollEvent(&event)) {
		switch (event.type) {
			...
			case SDL_EVENT_KEY_DOWN:
				...
				if (!event.key.repeat) {
					int game_code = decode_keysym(event.key.key);
					//printf("KEY_MAP=%d\n", game_code);
					if (game_code >= 0) {

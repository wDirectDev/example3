...
#include "SDL2_gfxFontPrimitives.h"
...
int characterRGBA(SDL_Renderer *renderer, Sint16 x, Sint16 y, char c, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	if (gfxPrimitivesFont[ci] == NULL) {
		...
		*/
		...
		character =	SDL_CreateSurface(
			charWidth, charHeight, SDL_GetPixelFormatForMasks(32, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF) );
	...
	*/
	...
	rc = SDL_SetTextureColorMod(gfxPrimitivesFont[ci], r, g, b);
	if ( !rc ) {
		printf( "ERROR: SDL_SetTextureColorMod %s\n", SDL_GetError() );
		return -1;
	}

	rc = SDL_SetTextureAlphaMod(gfxPrimitivesFont[ci], a);
	if ( !rc ) {
		printf( "ERROR: SDL_SetTextureAlphaMod %s\n", SDL_GetError() );
		return -1;
	}
	...
	*/
	rc = SDL_RenderTexture(renderer, gfxPrimitivesFont[ci], &srect, &drect);
	if ( !rc ) {
		printf( "ERROR: SDL_RenderTexture %s\n", SDL_GetError() );
		return -1;
	}

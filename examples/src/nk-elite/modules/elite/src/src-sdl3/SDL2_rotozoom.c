...
SDL_Surface* rotateSurface90Degrees(SDL_Surface* src, int numClockwiseTurns) 
{
	...
	Uint32 Bmask = 0;
	Uint32 Amask = 0;
	SDL_GetMasksForPixelFormat(src->format, &byteperpixel, &Rmask, &Gmask, &Bmask, &Amask);
	if ((byteperpixel % 8) != 0) {
	...
	SDL_GetMasksForPixelFormat(src->format, &byteperpixel, &Rmask, &Gmask, &Bmask, &Amask);
	dst = SDL_CreateSurface( newWidth, newHeight, SDL_GetPixelFormatForMasks(byteperpixel, Rmask, Gmask, Bmask, Amask) );
	if(!dst) {
...
SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double angle, double zoomx, double zoomy, int smooth)
{
	SDL_Surface *rz_src;
	SDL_Surface *rz_dst;
	SDL_Palette *palette_src;
	SDL_Palette *palette_dst;
	double zoominv;
	...
	*/
	...
	Uint32 Amask = 0;
	SDL_GetMasksForPixelFormat(src->format, &byteperpixel, &Rmask, &Gmask, &Bmask, &Amask);
	is32bit = (byteperpixel == 32);
	if ((is32bit) || (byteperpixel == 8)) {
		/*
		...
		*/
		rz_src =
			SDL_CreateSurface(src->w, src->h, SDL_GetPixelFormatForMasks( 32, 
	...
	if (fabs(angle) > VALUE_LIMIT) {
		...
		*/
		...
		if (is32bit) {
			...
			*/
			...
			Uint32 Bmask = 0;
			Uint32 Amask = 0;
			SDL_GetMasksForPixelFormat(rz_src->format, &byteperpixel, &Rmask, &Gmask, &Bmask, &Amask);
			rz_dst =
				SDL_CreateSurface(dstwidth, dstheight + GUARD_ROWS, SDL_GetPixelFormatForMasks(byteperpixel, Rmask, Gmask, Bmask, Amask));
		} else {
			/*
			* Target surface is 8bit 
			*/
			rz_dst = SDL_CreateSurface(dstwidth, dstheight + GUARD_ROWS, SDL_GetPixelFormatForMasks( 8, 0, 0, 0, 0 ));
		}
		...
		*/
		if (is32bit) {
			...
			*/
			palette_src = SDL_GetSurfacePalette(rz_src); 
			palette_dst = SDL_GetSurfacePalette(rz_dst); 
			for (i = 0; i < palette_src->ncolors; i++) {
				palette_dst->colors[i] = palette_src->colors[i];
			}
			palette_dst->ncolors = palette_src->ncolors;
			/*
...
SDL_Surface *shrinkSurface(SDL_Surface *src, int factorx, int factory)
{
	...
	*/
	...
	if ((is32bit) || (byteperpixel == 8)) {
		...
		* New source surface is 32bit with a defined RGBA ordering 
		*/
		rz_src = SDL_CreateSurface( src->w, src->h, SDL_GetPixelFormatForMasks(32, 
#if SDL_BYTEORDER == SDL_LIL_ENDIAN
			0x000000ff, 0x0000ff00, 0x00ff0000, 0xff000000
#else
			0xff000000,  0x00ff0000, 0x0000ff00, 0x000000ff
